...
namespace WordPatForm
{
    internal class Cipherword
    {
        ...
        public int Length
        {
            get { return _ciphertext.Length; }
        }

        public int HowManyPossibilities()
        {
            return _plaintextpossibilities == null ? 0 : _plaintextpossibilities.Count;
        }

        public Dictionary<char, char> SwapMatchingWordCharacters(Dictionary<char,char> dictionary )
        {
            if (_ciphertext.AllCaps())
            {  
            ...
            foreach (var codewordtext in _plaintextpossibilities)
            {
                ...
                var match = true;
                for (var i = 0; i < _ciphertext.Length; i++)  //for each character in the ciphertext 
                {

                    if (!char.IsUpper(_ciphertext[i]))// if the cipher text is lowercase
                    {
                    ...
                    if (!char.IsUpper(_ciphertext[i])) continue;
                    if (codewordtext[i] != _ciphertext[i])
        ...
        private Dictionary<char, char> ReturnSingleListItemCharacterDictionary()
        {
            ...
            for (int x = 0; x < Length; x++)  //for the length of the cipherword text 
            {
                if (!dictionary.ContainsKey(_plaintext[x]))  // if the character dictionary does not contain a key
                    dictionary.Add(_plaintext[x], _plaintextpossibilities[0][x]);  //at the first element at the specified location add the character 
            }
        ...
        public override string ToString()
        {
            ...
            int i = HowManyPossibilities();
            if (i < 1)
            {
                return _ciphertext + " " + _pattern;
            }
            if (i == 1)
            {
            ...
            return _ciphertext + " " + _pattern + " " + words;
        }
    }
}